    ...
    while True:
        ...
        try:
            ...
            if "SearchResultFragmentNew" in output:
                ...
                await asyncio.sleep(1)
                device.shell(f"{d['keyevents']['enter']}")
                await asyncio.sleep(2)
                device.shell(f"input tap {d['xy_params']['users_tab']}")
                await asyncio.sleep(1)
                device.shell(f"input tap {d['xy_params']['top_result']}")
                await asyncio.sleep(1)

            if "UserProfileFragment" in output:
                print(f"In Profile")
                await asyncio.sleep(1)
                device.shell(f"{d['keyevents']['tab']}")
                await asyncio.sleep(1)
                device.shell(f"{d['keyevents']['up']}")
                ...
                device.shell(f"{d['keyevents']['up']}")
                await asyncio.sleep(1)
                device.shell(f"{d['keyevents']['down']}")
                await asyncio.sleep(1)
                device.shell(f"{d['keyevents']['down']}")
                await asyncio.sleep(1)
                device.shell(f"{d['keyevents']['down']}")
